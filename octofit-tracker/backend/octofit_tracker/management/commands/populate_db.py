...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        # Modify the duration field to store total seconds instead of timedelta objects
        activities = [
            {"_id": ObjectId(), "user": users[0]["_id"], "activity_type": "Cycling", "duration": int(timedelta(hours=1).total_seconds())},
            {"_id": ObjectId(), "user": users[1]["_id"], "activity_type": "Crossfit", "duration": int(timedelta(hours=2).total_seconds())},
            {"_id": ObjectId(), "user": users[2]["_id"], "activity_type": "Running", "duration": int(timedelta(hours=1, minutes=30).total_seconds())},
            {"_id": ObjectId(), "user": users[3]["_id"], "activity_type": "Strength", "duration": int(timedelta(minutes=30).total_seconds())},
            {"_id": ObjectId(), "user": users[4]["_id"], "activity_type": "Swimming", "duration": int(timedelta(hours=1, minutes=15).total_seconds())},
        ]
